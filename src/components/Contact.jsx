...
const Contact = ({ setShowpopup }) => {
  const dets = [{
    icon: <MdCall className='m-4 text-(--primary-color)' />,
    title: 'phone number',
    ...
    icon: <MdLocalPostOffice className='m-4 text-(--primary-color) ' />,
    title: 'Email address',
  ...
  return (
    <div className='bg-white p-8 '>
      <h1 className="text-(--primary-color) mb-5 lg:text-5xl text-3xl">Contact Now</h1>
      ...
      <div id='contact' className="lg:flex lg:flex-row" >
        <div >
          <div className=' grid grid-cols-2 gap-1 lg:gap-4 lg:pr-8 lg:h-full '>
            {dets.map((det, index) => (
              <div onClick={det.onclick} className='p-2 lg:h-full rounded bg-white border border-(--primary-color)/10 flex flex-col justify-center items-center shadow-lg shadow-(--gray) hover:scale-[102%] duration-300 text-center' key={index}>
